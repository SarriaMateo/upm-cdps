...
Creamos las imágenes de `productpage`, `details`, `ratings` y las de cada versión de `reviews` y las subimos a nuestro repositorio de Docker-hub: `mateosarria/pc2`. Las imágenes siguen una nomenclatura de tipo `mateosarria/pc2:(servicio)` por ejemplo para productpage: sandracascantem/35:productpage, para la versión 1 de reviews: sandracascantem/35:reviews-v1, (indicado en los ficheros .yaml correspondientes). (No fue posible en Docker-hub respetar el nombre de las imágenes 35/servicio).
à
